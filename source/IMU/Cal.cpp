...
void Demarcate(const double sixpos[6][3], double acc_bias[3], double acc_scale[3],double acc_m[6])
{
	...
	Eigen::Matrix<double, 4, 6> A = Eigen::Matrix<double, 4, 6>::Zero();
	//L矩阵赋值
	for (int i = 0; i < 6; i++)
	{
		L(0, i) = sixpos[i][0];
	...
	//M矩阵计算
	Eigen::Matrix<double, 4, 4> N = A * A.transpose();
	Eigen::Matrix<double, 4, 4> N_inv = N.inverse();
	M = L * A.transpose() * N_inv;

	// 新增：计算残差
	Eigen::Matrix<double, 3, 6> L_calc = M * A;
	Eigen::Matrix<double, 3, 6> residual = L - L_calc;
...
//g_n,v_n,g_b,v_b,其中n,b为上标
	Eigen::Vector3d g_n = Eigen::Vector3d::Zero();
	Eigen::Vector3d w_n_ie = Eigen::Vector3d::Zero();
	Eigen::Vector3d v_n = Eigen::Vector3d::Zero();
	Eigen::Vector3d g_b = Eigen::Vector3d::Zero();
	Eigen::Vector3d w_b_ie = Eigen::Vector3d::Zero();
	Eigen::Vector3d v_b = Eigen::Vector3d::Zero();
	Eigen::Matrix3d C_n_b = Eigen::Matrix3d::Zero();
	...
	g_n(2) = g;
	//w_n_ie
	w_n_ie(0) = Omegae * cos(local_fai * deg2rad_scale);
	w_n_ie(2) = -Omegae * sin(local_fai * deg2rad_scale);
	//v_n=g_n x w_n_ie
	v_n = g_n.cross(w_n_ie);
	//g_b
	g_b(0) = -acc_mean[0];
	g_b(1) = -acc_mean[1];
	g_b(2) = -acc_mean[2];
	//w_b_ie
	w_b_ie(0) = gyro_mean[0];
	w_b_ie(1) = gyro_mean[1];
	w_b_ie(2) = gyro_mean[2];
	//v_b=g_b x w_b_ie
	v_b = g_b.cross(w_b_ie);


	//-------------------矢量正交化和单位化---------------------
	// 检查 v 系向量是否平行
	if (g_n.cross(w_n_ie).norm() < 1e-10)
	{
		throw std::runtime_error("导航系：重力向量和角速度向量平行，无法构建正交系");
	}

	// 检查 w 系向量是否平行  
	if (g_b.cross(w_b_ie).norm() < 1e-10)
	{
		throw std::runtime_error("载体系：重力向量和角速度向量平行，无法构建正交系");
	}
	//定义向量
	Eigen::Vector3d v_g = Eigen::Vector3d::Zero();
	Eigen::Vector3d v_w = Eigen::Vector3d::Zero();
	Eigen::Vector3d v_gw = Eigen::Vector3d::Zero();
	Eigen::Vector3d w_g = Eigen::Vector3d::Zero();
	Eigen::Vector3d w_w = Eigen::Vector3d::Zero();
	Eigen::Vector3d w_gw = Eigen::Vector3d::Zero();
	//正交化和单位化
	v_g = g_n / g_n.norm();
	v_w = v_g.cross(w_n_ie);
	v_w = v_w / v_w.norm();
	v_gw = v_w.cross(v_g);

	w_g = g_b / g_b.norm();
	w_w = w_g.cross(w_b_ie);
	w_w = w_w / w_w.norm();
	w_gw = w_w.cross(w_g);
	...
	//----------------------构造辅助矩阵-------------------------
	Eigen::Matrix3d gwv_n = Eigen::Matrix3d::Zero();
	Eigen::Matrix3d gwv_b = Eigen::Matrix3d::Zero();
	//初始化
	for (int i = 0; i < 3; i++)
	{
		//gwv_n
	...
	C_n_b = gwv_n * gwv_b;
	// 检查 C_n_b 是否正交
	Eigen::Matrix3d I_check = C_n_b * C_n_b.transpose();
...
{
	//-----------------------------矩阵，向量定义-----------------------------
	Eigen::Vector3d fixed_acc = Eigen::Vector3d::Zero();
	Eigen::Vector3d raw_acc = Eigen::Vector3d::Zero();
	Eigen::Matrix3d fix_Mat = Eigen::Matrix3d::Zero();
	...
	Eigen::MatrixXd fix_Mat_inv = fix_Mat.inverse();
...
void SPCA_func()
{
	...
	/*const char* SPCAdataname = "D:\\2026大三下DHGC实习\\6.5\\静态数据.txt";
	IMUdataRead(SPCAdataname, SPCA_data);*/
