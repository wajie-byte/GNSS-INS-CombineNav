	...
	double BLHk[3], double Vk_n[3], double YPRk[3], Eigen::Vector4d& q_bn_k)
{
	...
	//计算姿态矩阵
	Eigen::Matrix3d C_bn_k_1 = quaternion2C_b_n(q_bn_k_1);
	//计算比力积分项
	...
	//计算n系的旋转四元数
	Cal_ERV_in(w_ie_n_mid, w_en_n_mid, dt, ERV_in);
	Eigen::Vector4d q_n_rotate = Cal_neg_rotate(ERV_in);//n系姿态变化四元数
	//计算b系的旋转四元数(补偿圆锥效应)
	double ERV_ib[3] = { 0.0,0.0,0.0 };
	Cal_ERV_ib(gyro_k_1, gyro_k, ERV_ib);
	Eigen::Vector4d q_b_rotate = Cal_pos_rotate(ERV_ib);
	//姿态更新
	...
	std::vector<INS::IMUResultEpoch>& imuresult)
{
	...
	Eigen::Vector4d q_bn_k_1 = euler2quaternion(imustart.YPR[0], imustart.YPR[1], imustart.YPR[2]);//k-1时刻姿态四元数
	//k时刻
