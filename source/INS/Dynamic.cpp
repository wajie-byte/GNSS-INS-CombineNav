...
*/
Eigen::Matrix3d Cal_C_b_n(double Yaw, double Pitch, double Roll)
{
	...
	double c = Yaw;
	//定义姿态矩阵
	Eigen::Matrix3d C_b_n = Eigen::Matrix3d::Zero();
	//姿态矩阵元素计算
...
*/
Eigen::Vector4d euler2quaternion(double Yaw, double Pitch, double Roll)
{
	//计算半倍欧拉角对应的正弦余弦值
	...
	//q_b^n=q_z*q_y*q_x
	Eigen::Vector4d q_b_n = Eigen::Vector4d::Zero();
	//元素计算
...
Out:
	Mat C_b^n			//姿态矩阵,b为下标，n为上标(3x3)
*/
Eigen::Matrix3d quaternion2C_b_n(const Eigen::Vector4d& q_b_n)
{
	//获取四元数四个元的值进行求解
	double qw = q_b_n(0, 0);
	double qx = q_b_n(1, 0);
...
Out:
	...
	double Roll			//横滚角rad		
*/
void quaternion2euler(const Eigen::Vector4d& q_b_n, double& Yaw, double& Pitch, double& Roll)
{
	//获取四元数四个元的值进行求解
	double qw = q_b_n(0, 0);
	double qx = q_b_n(1, 0);
...
*/
Eigen::Vector4d Cal_pos_rotate(const double ERV[3])
{
	double angle = sqrt(ERV[0] * ERV[0] + ERV[1] * ERV[1] + ERV[2] * ERV[2]);

	Eigen::Vector4d q_b_rotate = Eigen::Vector4d::Zero();
...
*/
Eigen::Vector4d Cal_neg_rotate(const double ERV[3])
{
	// 计算总角度
	double angle = sqrt(ERV[0] * ERV[0] + ERV[1] * ERV[1] + ERV[2] * ERV[2]);

	Eigen::Vector4d q_n_rotate = Eigen::Vector4d::Zero();
...
Out:
	Mat c			//四元数c(4x1)
*/
Eigen::Vector4d quaternion_multiply(const Eigen::Vector4d& a, const Eigen::Vector4d& b)
{
...
*/
Eigen::Vector4d Attupdate_quaternion(const Eigen::Vector4d& q_n_rotate, const Eigen::Vector4d& q_bn_k_1, const Eigen::Vector4d& q_b_rotate)
{
	Eigen::Vector4d q_bn_k = Eigen::Vector4d::Zero();
	Eigen::Vector4d temp = quaternion_multiply(q_bn_k_1, q_b_rotate);
...
Out:
	Mat b				//归一化后四元数
*/
Eigen::Vector4d quaternion_normalize(const Eigen::Vector4d& a)
{
...
*/
void Cal_dv_f_n(const double ERV[3], const Eigen::Matrix3d& C_bn_k_1, const double dv_f_b[3],
	double dv_f_n[3])
{
	Eigen::Matrix3d I = Eigen::Matrix3d::Identity();//3x3单位矩阵
	Eigen::MatrixXd antisym_ERV = Cal_Antisymmmetric_Mat(ERV);//等效旋转矢量的反对称矩阵
	Eigen::Vector3d dv_fb = Eigen::Vector3d::Zero();//将b系比力积分项用Mat类表示，便于后续矩阵运算
	for (int i = 0; i < 3; i++)
	{
		dv_fb(i, 0) = dv_f_b[i];
	}
	Eigen::Matrix3d I_ERV = I - 0.5 * antisym_ERV;
	Eigen::Vector3d dv_fn = I_ERV * C_bn_k_1 * dv_fb;
	for (int i = 0; i < 3; i++)
	{
		dv_f_n[i] = dv_fn(i, 0);
	}
...
Out:
	Mat ax					//反对称矩阵
*/
Eigen::Matrix3d Cal_Antisymmmetric_Mat(const double a[3])
{
...
Out:
	Mat ax					//反对称矩阵
*/
Eigen::Matrix3d Skew(const double a[3])
{
...
Out:
	Mat qne
*/
Eigen::Vector4d Cal_qne(const double BLH[3])
{
	...
	Eigen::Vector4d qne = Eigen::Vector4d::Zero();
...
Out:
	Mat D			//对角矩阵
*/
Eigen::Matrix3d diag(const double a[3])
{
